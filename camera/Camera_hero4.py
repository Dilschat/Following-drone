...
class Camera_hero4(Camera):
    ...
    def __init__(self, tracker, FPS, connection, resize):
        ...
        self.skip = 0
        self.KEEP_ALIVE_PERIOD = 25*100
        self.KEEP_ALIVE_CMD = 2
        self.MESSAGE = self.get_command_msg(self.KEEP_ALIVE_CMD)
    ...
    def start(self):
        ...
        self.__cap = cv2.VideoCapture("udp://:8554", cv2.CAP_FFMPEG)
    ...
    def run(self):
        ...
        while True:
            ...
            if ret:
                img = self.img_resize(img_raw, factor=self.resize_factor)
                ...
                if current_time - last_message >= self.KEEP_ALIVE_PERIOD/1000:
                   print("Sending keep alive message to")
                   self.sock.sendto(self.MESSAGE, ("10.5.5.9", 8554))
